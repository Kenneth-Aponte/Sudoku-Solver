...
public class InputState extends State {
	...
	public boolean showingInstructions = false;
    ...
    public void tick() {
    	...
    	if(handler.getKeyManager().keyJustPressed(KeyEvent.VK_H)) {
    		showingInstructions = !showingInstructions;//toggles it
    		
    	}
    	
    	if(!showingInstructions) {//wont allow the player to make changes if the instructions are shown in the screen
    ...
    public void render(Graphics g) {
        ...
        if(showingInstructions) {//I tried using \n on a single line but didn't work
        	g.drawString("Instructions:", handler.getWidth()/8, handler.getHeight()/8);
        	g.drawString("- Use the arrow keys to move throughout the grid.",handler.getWidth()/8, handler.getHeight()/8 + 48*1);
        	g.drawString("- Insert the corresponding numbers by typing them on your keyboard.",handler.getWidth()/8, handler.getHeight()/8 + 48*2);
        	g.drawString("- To remove a specific number in a square, press 0 while on that square.",handler.getWidth()/8, handler.getHeight()/8 + 48*3);
        	g.drawString("- To refresh the grid press R.",handler.getWidth()/8, handler.getHeight()/8 + 48*4);
        	g.drawString("- When ready press ENTER to solve the Sudoku.",handler.getWidth()/8, handler.getHeight()/8 + 48*5);
        	g.drawString("Enjoy!",handler.getWidth()/8, handler.getHeight()/8 + 48*6);
        }
        else {
        	g.drawString("PRESS H FOR INSTRUCTIONS", handler.getWidth()/8, handler.getHeight()/8);
        	//Main Grid------------------------------------------------------------------
            //horizontal
